...
import type { DataTableAdvancedFilterField, Filter, FilterOperator, JoinOperator, StringKeyOf } from "@lib/types";
...
import { getDefaultFilterOperator, getFilterOperators } from "@lib/helpers/data-table";
import { getFiltersStateParser } from "@lib/helpers/parsers";
...
export function DataTableFilterList<TData>({ table, filterFields, debounceMs, shallow }: DataTableFilterListProps<TData>) {
  ...
  const onFilterAdd = React.useCallback(() => {
    ...
    void setFilters([
      ...filters,
      {
        ...
        filterId: customAlphabet("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz", 6)()
      }
    ]);
  }, [filterFields, filters, setFilters]);

  const onFilterUpdate = React.useCallback(
    ({ filterId, field, debounced = false }: { filterId: string; field: Omit<Partial<Filter<TData>>, "filterId">; debounced?: boolean }) => {
    ...
    ({ filter, inputId }: { filter: Filter<TData>; inputId: string }) => {
      ...
      switch (filter.type) {
          ...
          return (
            ...
            >
              <FacetedTrigger asChild>
                ...
                >
                  <FacetedBadgeList options={filterField.options} placeholder={filterField.placeholder ?? "Select an option..."} />
                </Button>
              </FacetedTrigger>
              <FacetedContent id={`${inputId}-listbox`} className="w-[12.5rem] origin-(--radix-popover-content-transform-origin)">
                <FacetedInput aria-label={`Search ${filterField?.label} options`} />
                <FacetedList>
            ...
            >
              <FacetedTrigger asChild>
                <Button
                  ...
                  className="w-full rounded px-1.5"
                >
                  <FacetedBadgeList options={filterField.options} placeholder={filterField.placeholder ?? " Select options..."} />
                </Button>
              </FacetedTrigger>
        ...
        case "boolean": {
          ...
          return (
            <Select value={filter.value} onValueChange={value => onFilterUpdate({ filterId: filter.filterId, field: { value } })}>
              <SelectTrigger
                id={inputId}
                aria-controls={`${inputId}-listbox`}
  ...
  return (
    <Sortable value={filters} getItemValue={item => item.filterId} onMove={({ activeIndex, overIndex }) => onFilterMove(activeIndex, overIndex)}>
      <Popover>
        <PopoverTrigger asChild>
          <Button aria-label="Open filters" variant="outline" size="sm" className="[&>svg]:size-3">
            <ListFilter aria-hidden="true" />
            Filters
            {filters.length > 0 && (
              <Badge variant="secondary" className="h-[1.14rem] rounded-[0.2rem] px-[0.32rem] font-mono font-normal text-[0.65rem]">
                {filters.length}
              </Badge>
            )}
          </Button>
        ...
        >
          <div className="flex flex-col gap-1">
            ...
            <p id={descriptionId} className={cn("text-muted-foreground text-sm", filters.length > 0 && "sr-only")}>
              {filters.length > 0 ? "Modify filters to refine your results." : "Add filters to refine your results."}
            </p>
          </div>
          {filters.length > 0 ? (
            <SortableContent asChild>
              <div role="list" className="flex max-h-[300px] flex-col gap-2 overflow-y-auto py-0.5 pr-1">
                {filters.map((filter, index) => {
                  ...
                  return (
                    <SortableItem key={filter.filterId} value={filter.filterId} asChild>
                      <div role="listitem" id={filterItemId} tabIndex={-1} className="flex items-center gap-2">
                        <div className="min-w-[4.5rem] text-center">
                          {index === 0 ? (
                            ...
                            <Select value={joinOperator} onValueChange={(value: JoinOperator) => setJoinOperator(value)}>
                              ...
                              <SelectContent id={joinOperatorListboxId} position="popper" className="min-w-(--radix-select-trigger-width) lowercase">
                                ...
                                <CommandGroup>
                                  {filterFields.map(field => (
                                    ...
                                    >
                                      ...
                                      <Check className={cn("ml-auto size-4 shrink-0", field.id === filter.id ? "opacity-100" : "opacity-0")} />
                                    </CommandItem>
                                  ))}
                                </CommandGroup>
                              </CommandList>
                        ...
                        >
                          ...
                          <SelectContent id={operatorListboxId} className="origin-(--radix-select-content-transform-origin)">
                        ...
                        <div className="min-w-36 flex-1">{onFilterInputRender({ filter, inputId })}</div>
                        <Button
                        ...
                        <SortableItemHandle asChild>
                          <Button variant="outline" size="icon" className="size-8 shrink-0 rounded [&>svg]:size-3.5">
                            <GripVertical />
                          </Button>
                        </SortableItemHandle>
                      </div>
          ...
          <div className="flex w-full items-center gap-2">
            <Button size="sm" className="h-[1.85rem] rounded" onClick={onFilterAdd}>
              Add filter
            </Button>
            {filters.length > 0 ? (
              <Button
