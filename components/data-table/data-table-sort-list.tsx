...
export function DataTableSortList<TData>({ table, debounceMs, shallow }: DataTableSortListProps<TData>) {
  ...
  const initialSorting = React.useMemo(() => (table.initialState.sorting ?? []) as ExtendedSortingState<TData>, [table.initialState.sorting]);
  ...
  const [sorting, setSorting] = useQueryState<any[]>("sort", sortingParser);
  ...
  const onUpdateSort = React.useCallback(
    ({ id, field, debounced = false }: { id: string; field: Partial<ExtendedColumnSort<TData>>; debounced?: boolean }) => {
      const updateFunction = debounced ? debouncedSetSorting : setSorting;

      updateFunction(prevSorting => {
        if (!prevSorting) return prevSorting;

        return prevSorting.map(sort => (sort.id === id ? { ...sort, ...field } : sort));
      });
    },
  ...
  const onRemoveSort = React.useCallback(
    (id: string) => {
      void setSorting(prevSorting => prevSorting.filter(item => item.id !== id));
    },
    [setSorting]
  );

  const onSortingReset = React.useCallback(() => setSorting(null), [setSorting]);

  return (
    <Sortable value={sorting} onValueChange={setSorting} getItemValue={item => item.id}>
      <Popover>
        <PopoverTrigger asChild>
          <Button aria-label="Open sorting" variant="outline" size="sm" className="[&>svg]:size-3">
            <ArrowDownUp />
            Sort
        ...
        >
          <div className="flex flex-col gap-1">
            ...
            <p id={descriptionId} className={cn("text-muted-foreground text-sm", uniqueSorting.length > 0 && "sr-only")}>
              {uniqueSorting.length > 0 ? "Modify sorting to organize your results." : "Add sorting to organize your results."}
            </p>
          </div>
          {uniqueSorting.length > 0 ? (
            <SortableContent asChild>
              <div role="list" className="flex max-h-[300px] flex-col gap-2 overflow-y-auto p-0.5">
                {uniqueSorting.map(sort => {
                  ...
                  return (
                    <SortableItem key={sort.id} value={sort.id} asChild>
                      <div role="listitem" id={sortItemId} tabIndex={-1} className="flex items-center gap-2">
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button
                              id={fieldTriggerId}
                            ...
                            >
                              ...
                              <div className="ml-auto flex items-center gap-1">
                                {initialSorting[0]?.id === sort.id ? (
                                  <Badge variant="secondary" className="h-[1.125rem] rounded px-1 font-mono font-normal text-[0.65rem]">
                                    Default
                                  </Badge>
                                ) : null}
                          ...
                          <PopoverContent
                            ...
                            onCloseAutoFocus={() => document.getElementById(fieldTriggerId)?.focus({ preventScroll: true })}
                          >
                            <Command>
                                    ...
                                    <CommandItem
                                      ...
                                      onSelect={value => {
                                        ...
                                        requestAnimationFrame(() => {
                                          document.getElementById(newFieldTriggerId)?.focus();
                                        });
                                      }}
                                    >
                                      <span className="truncate">{column.label}</span>
                                      <Check
                                        className={cn("ml-auto size-4 shrink-0", column.id === sort.id ? "opacity-100" : "opacity-0")}
                                        aria-hidden="true"
                                      />
                                    </CommandItem>
                                  ))}
          ...
          <div className="flex w-full items-center gap-2">
            <Button size="sm" className="h-[1.85rem] rounded" onClick={onAddSort} disabled={uniqueSorting.length >= sortableColumns.length}>
              Add sort
            </Button>
            {uniqueSorting.length > 0 ? (
              <Button size="sm" variant="outline" className="rounded" onClick={onSortingReset}>
                Reset sorting
              </Button>
            ) : null}
          </div>
