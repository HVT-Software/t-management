import type { ExtendedSortingState, Filter } from "@/lib/types";
  ...
  return createParser<Filter<T>[]>({
    parse: value => {
      try {
        ...
        return result.data as any as Filter<T>[];
      } catch {
        return null;
      }
    },
    ...
    eq: (a, b) =>
      ...
      a.every(
        (filter, index) =>
          filter.id === b[index]?.id && filter.value === b[index]?.value && filter.type === b[index]?.type && filter.operator === b[index]?.operator
      )
  });
};
